...
│       ├── sidebar.py              # 侧边栏
│       ├── editor_panel.py         # 编辑区
│       ├── agent_panel.py          # Agent 面板（对话、Markdown、持久化）
...
用户配置存储在 `~/NovelWriter/config.json`：
...
└── AgentPanel       # Agent 选择、对话、快捷操作、聊天持久化
```

- `AgentWorker(QThread)`：后台线程执行 Agent 调用，信号驱动 UI 更新
- 流式输出：`chunk_received` 信号逐块更新 Markdown 渲染
- 聊天记录：`save_history()` / `load_history()` 持久化到 `~/NovelWriter/chat_history.json`
