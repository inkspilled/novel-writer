...
写作时自动组装 8 层上下文：
1. **大世界状态**：人物境界/背包/位置/物品/时间线（world_state.py）
2. **长期记忆**：11个桶的结构化记忆（memory.py）
3. **反模式约束**：检测并约束重复套路（anti_patterns.py）
4. **追读力指导**：分析章节追读力，生成写作指导（reading_power.py）
5. **角色推演**：推演角色自然反应（character_sim.py）
6. **章节概要**：最近10章的结构化概要卡片
7. **剧情摘要**：最新摘要 + 最近3章全文
8. **RAG 检索**：从早期章节检索相关段落（rag.py）
