...
│  侧边栏   │     编辑区        │   智能体办公室（占 50%+）   │
│          │                  │                           │
│ 项目管理   │  正文 / 大纲 / 备注 │  办公室场景（40%）+ 动画    │
│ 章节树    │  实时字数统计      │  工作流进度条 + 执行日志     │
│ 字数统计   │                  │  💬 对话区（60%）           │
└──────────┴──────────────────┴───────────────────────────┘
```
...
工作流执行时，当前步骤的 Agent 有醒目的脉冲光圈和任务标签，其余 Agent 显示等待状态。
...
- **办公室联动**：工作流执行时，办公室里的 Agent 实时反映状态（脉冲光圈 + 任务标签）
- **执行日志**：可展开的日志区，实时显示每步的开始/完成/出错
- **断点恢复**：中断后自动从上次进度继续
