...
6. 📑 **目录更新**（每章）→ 更新 planning/目录.md
7. ✨ **润色**（每2章）→ 提升文笔
8. 📝 **校验**（每章）→ 错别字检查
9. 📋 **摘要**（每5章）→ 剧情摘要
10. 🔄 **规划反哺**（每10章）→ 更新大纲/人物/伏笔
11. 🔍 **审核**（最后）→ 剧情逻辑审查

### 智能上下文

写作时自动组装 6 层上下文：
1. **长期记忆**：11个桶的结构化记忆（memory.py）
2. **反模式约束**：检测并约束重复套路（anti_patterns.py）
3. **追读力指导**：分析章节追读力，生成写作指导（reading_power.py）
4. **角色推演**：推演角色自然反应（character_sim.py）
5. **章节概要**：最近10章的结构化概要卡片
6. **剧情摘要**：最新摘要 + 最近3章全文
7. **RAG 检索**：从早期章节检索相关段落（rag.py）
