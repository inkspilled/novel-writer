...
写作时按任务类型分层组装上下文，自动裁剪不必要的信息：

| 层级 | 内容 | 适用步骤 |
|------|------|----------|
| Tier 0 (GLOBAL) | 项目元信息 | 校对、工具步骤 |
| Tier 1 (WORLD) | + 世界设定/角色状态/角色约束 | 规划、推演 |
| Tier 2 (NARRATIVE) | + 剧情摘要/伏笔/反模式/追读力 | 灵感、润色、审核 |
| Tier 3 (WORKING) | + 章节概要/全文/推演/RAG | 章节写作 |

上下文自动去重（合并 world_state + memory 角色数据），并拆分为稳定前缀（可缓存）和变量后缀，优化 Prompt Caching 命中率。
